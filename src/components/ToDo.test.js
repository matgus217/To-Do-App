...
describe("<ToDo/>", () => {
  ...
  describe("The default UI", () => {
    it("Renders two default todo items", () => {
      ...
      expect(getByText("Cook food")).toBeInTheDocument();
    });
  ...
  describe("Deleting items", () => {
    ...
    it("means that because the first toDoItem was deleted, the first toDoItem should now be cook food", () => {
      const { queryAllByTestId, queryByTestId } = render(<ToDo />);
      const deleteButtons = queryAllByTestId("delete");
      fireEvent.click(deleteButtons[0]);
      const todoText = queryByTestId("todo-text");
      expect(todoText.textContent).toEqual("Cook food");
    });
  });
});